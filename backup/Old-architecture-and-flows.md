# Raza Architecture & Flows

## Executive Summary

Raza is a personalized song generation platform where users record their voice and create custom songs (MVP: 45-60 seconds, Full: 45-90 seconds) that sound like them singing. This document captures both MVP and full-product architecture and flows.
...
│  │  • Full Render Workflow (extended preview with billing)                           │   │
...
│                                                                         │
│ ┌─────────────────────────────────────────────────────────────────────┐ │
│ │ ☑ I consent to Raza creating a voice model from my recordings       │ │
│ │ ☑ I understand my voice stays on Raza's secure servers              │ │
...
### Overview
User provides a message and occasion → System generates lyrics → User reviews/edits → System generates preview → User confirms → Full song rendered.
...
│ Message     │        │ Generation  │       │ + Voice     │       │ + Billing   │
│ Style       │        │ + Review    │       │ Conversion  │       │ + Delivery  │
└─────────────┘        └─────────────┘       └─────────────┘       └─────────────┘
     │                      │                      │                      │
     ▼                      ▼                      ▼                      ▼
┌─────────────┐        ┌─────────────┐       ┌─────────────┐       ┌─────────────┐
│ • Birthday  │        │ • Singable  │       │ • 15-25 sec │       │ • 45-90 sec │
│ • Thank you │        │   syllables │       │   chorus    │       │   full song │
│ • I love you│        │ • Name in   │       │ • p95 < 90s │       │ • Credits   │
│ • Custom    │        │   chorus    │       │   latency   │       │   charged   │
└─────────────┘        └─────────────┘       └─────────────┘       └─────────────┘
```
...
└─────────────────────────────────────────────────────────────────────────────────────────┘
      ...
      Temporal Workflow Steps (p95 target: 90 seconds)
                    │
    ┌───────────────┼───────────────┬───────────────┬───────────────┐
    ▼               ▼               ▼               ▼               ▼
┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐
...
│     To save this song, download the Raza app.                           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
...
│     │ Download the Raza app to save it to your library.         │      │
│     │                                                           │      │
...
1. **Phase 0 Validation**
   ...
   - Validate end-to-end latency meets p95 < 90s target
