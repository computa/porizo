...
-- Adds: account_deleted (deletion now emits a retained security-timeline event)

-- Drop the existing constraint
ALTER TABLE auth_events DROP CONSTRAINT IF EXISTS auth_events_event_type_check;

-- Add new constraint with extended event types (all previously-allowed values + account_deleted)
