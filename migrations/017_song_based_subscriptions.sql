...
VALUES
  ('free', 'Free', 'free', 0, 5, 0, 0, 'Try Porizo with limited previews', '["5 previews per day","No full songs"]', 1, 0, datetime('now'), datetime('now')),
  ('plus', 'Plus', 'plus', 4, 20, 999, 9999, 'Perfect for occasional gifting', '["4 songs per month","20 previews per day","All occasions","All music styles"]', 1, 1, datetime('now'), datetime('now')),
  ('pro', 'Pro', 'pro', 10, -1, 1499, 14999, 'For power users and families', '["10 songs per month","Unlimited previews","All occasions","All music styles","Priority processing"]', 1, 2, datetime('now'), datetime('now'));

-- Insert default plan products (Apple)
INSERT INTO plan_products (id, plan_id, platform, product_id, billing_period, created_at)
VALUES
  ('apple_plus_monthly', 'plus', 'apple', 'com.porizo.plus_monthly', 'monthly', datetime('now')),
  ('apple_plus_annual', 'plus', 'apple', 'com.porizo.plus_annual', 'annual', datetime('now')),
  ('apple_pro_monthly', 'pro', 'apple', 'com.porizo.pro_monthly', 'monthly', datetime('now')),
  ('apple_pro_annual', 'pro', 'apple', 'com.porizo.pro_annual', 'annual', datetime('now'));

-- Insert default plan products (Google)
INSERT INTO plan_products (id, plan_id, platform, product_id, billing_period, created_at)
VALUES
  ('google_plus_monthly', 'plus', 'google', 'plus_monthly', 'monthly', datetime('now')),
  ('google_plus_annual', 'plus', 'google', 'plus_annual', 'annual', datetime('now')),
  ('google_pro_monthly', 'pro', 'google', 'pro_monthly', 'monthly', datetime('now')),
  ('google_pro_annual', 'pro', 'google', 'pro_annual', 'annual', datetime('now'));

-- Insert default trial config
INSERT INTO trial_config (id, songs_allowed, duration_days, is_active, updated_at)
VALUES (1, 2, 7, 1, datetime('now'));
