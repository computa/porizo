...
describe("POST /story/:story_id/to-track contract", () => {
  test("preserves requested voice mode and canonical story provenance", async () => {
    ...
    writer.getStoryContext = async () => ({
      ...
      summary: {
        text: "Awka trained him to keep going.",
        factCount: 1,
        beatsUncovered: 0,
      },
      status: "confirmed",
    ...
    assert.equal(trackInsert.args[10], "user_voice");

    const storyContext = JSON.parse(trackInsert.args[9]);
    ...
    const versionInsert = executed.find((entry) =>
      entry.sql.includes("INSERT INTO track_versions"),
    );
  ...
  test("prefers explicit request style over stale story context style", async () => {
    ...
    writer.getStoryContext = async () => ({
      ...
      summary: {
        text: "She kept everyone together.",
        factCount: 1,
        beatsUncovered: 0,
      },
      status: "confirmed",
    ...
    assert.ok(trackInsert, "expected track insert");
    assert.equal(trackInsert.args[7], "igbo_highlife");
  ...
  test("preserves cited song_map contracts from story context through stored track into lyric prompt", async () => {
    ...
    writer.getStoryContext = async () => ({
      ...
      eventType: "tribute",
      initialPrompt:
        "She held the family together through fear and became the heart of the home.",
      narrative:
        "Chioma carried school runs, work calls, and the family through a frightening twin pregnancy. Watching her grow into a stronger woman deepened everyone's love and respect.",
      facts: [
        {
          id: "f_scene",
          text: "School runs and work calls filled every day",
          beat: "scene",
        },
        {
          id: "f_turn",
          text: "The high-risk twin pregnancy changed everything",
          beat: "turning_point",
        },
        {
          id: "f_meaning",
      ...
      song_map: {
        hook: {
          idea: "You made our house a home",
        ...
        chorus: [
          {
            idea: "What it meant was love under pressure",
            source_facts: ["f_meaning"],
          },
        ],
        verse2: [
          {
            idea: "The high-risk twin pregnancy changed everything",
            source_facts: ["f_turn"],
          },
        ],
        bridge: [
        ...
        key_lines: [
          { idea: "You made our house a home", source_facts: ["f_meaning"] },
        ],
      },
      summary: {
        text: "She carried the family through fear and became the heart of the home.",
        factCount: 3,
        beatsUncovered: 0,
      },
      status: "confirmed",
    ...
    const storedTrack = {
      ...
      style: trackInsert.args[7],
      message: trackInsert.args[8],
      story_context_json: trackInsert.args[9],
    };
    ...
    assert.deepStrictEqual(lyricsContext.song_map.hook.source_facts, [
      "f_meaning",
    ]);
    ...
    assert.match(
      prompt,
      /Support: School runs and work calls filled every day/,
    );
