...
describe('Token Rotation Race Condition', () => {
  ...
  test('should fail one of two concurrent refresh requests', async () => {
    ...
    if (failures.length > 0) {
      console.log('Failure reason: ' + (failures[0].reason?.code || failures[0].reason?.message));
    }
    ...
    assert.ok(
      ['TOKEN_ALREADY_ROTATED', 'TOKEN_ROTATION_CONFLICT'].includes(failureCode),
      'Expected TOKEN_ALREADY_ROTATED or TOKEN_ROTATION_CONFLICT, got: ' + failureCode
    );
