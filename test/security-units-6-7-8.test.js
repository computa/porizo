...
describe("Unit 6: Job retry status guard", () => {
  ...
  async function seedJob(trackVersionId, status = "failed") {
    ...
    await db
      .prepare("UPDATE track_versions SET preview_job_id = ? WHERE id = ?")
  ...
  test("status guard SQL: UPDATE with wrong status returns 0 changes", async () => {
    ...
    assert.equal(
      result.changes,
      0,
      "status guard should prevent update of running job",
    );
...
describe("Unit 7: Enrollment risk level check", () => {
  ...
  test("allows enrollment for medium-risk users", async () => {
    ...
    // Clear the per-user burst rate limit (1 enrollment_start / 15s) that the
    // setup call above consumed, so this assertion call isn't 429'd.
  ...
  test("blocks enrollment for high-risk users", async () => {
    ...
    assert.ok(
      body.message.includes("Voice features"),
      "should mention voice features",
    );
  });
  ...
  test("risk check runs before rate limit consumption", async () => {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      assert.equal(
        res.statusCode,
        403,
        `Request ${i + 1} should be 403 (blocked), not 429 (rate limited)`,
      );
    }
  });
});
