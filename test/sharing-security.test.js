...
describe("Sharing Security", () => {
  ...
  it("single claim succeeds with correct PIN and device token", async () => {
    ...
    const res = await app.inject({
      ...
      payload: { pin: claimPin, device_id: deviceId, platform: "ios" },
    });
    ...
    // Verify DB state
    const share = db.prepare("SELECT * FROM share_tokens WHERE id = ?").get(shareId);
  ...
  it("concurrent claims: second claim returns 409", async () => {
    ...
    const res1 = await app.inject({
      method: "POST",
      url: `/share/${shareId}/claim`,
      payload: { pin: claimPin, device_id: device1, platform: "ios" },
    });
    assert.strictEqual(res1.statusCode, 200, `First claim should succeed: ${res1.body}`);

    // Second claim — same share, different device
    const res2 = await app.inject({
      method: "POST",
      url: `/share/${shareId}/claim`,
      payload: { pin: claimPin, device_id: device2, platform: "android" },
    });
    assert.strictEqual(res2.statusCode, 409, `Second claim should get 409: ${res2.body}`);
  ...
  it("claim with wrong PIN does not return dl_token", async () => {
    ...
    const body = JSON.parse(res.body);
    assert.strictEqual(body.dl_token, undefined, "dl_token must not be present on failed PIN");
    assert.strictEqual(body.web_stream_url, undefined, "web_stream_url must not be present on failed PIN");
  });
  ...
  it("claim with empty PIN does not increment lockout counter", async () => {
    ...
    const shareBefore = db.prepare("SELECT claim_attempts FROM share_tokens WHERE id = ?").get(shareId);
    ...
    const shareAfter = db.prepare("SELECT claim_attempts FROM share_tokens WHERE id = ?").get(shareId);
    assert.strictEqual(shareAfter.claim_attempts, shareBefore.claim_attempts,
  ...
  it("generated PIN is exactly 6 digits", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      assert.strictEqual(claimPin.length, 6, `PIN should be 6 digits, got: ${claimPin}`);
      assert.match(claimPin, /^\d{6}$/, `PIN should be all digits: ${claimPin}`);
      const num = parseInt(claimPin, 10);
      assert.ok(num >= 100000 && num <= 999999, `PIN should be in [100000, 999999]: ${num}`);
    }
  });
  ...
  it("device token fallback is rejected in production mode", async () => {
    ...
    try {
      ...
      const res = await app.inject({
        ...
        payload: { pin: claimPin, device_id: "prod_device_123", platform: "ios" },
      });
      ...
      const share = db.prepare("SELECT * FROM share_tokens WHERE id = ?").get(shareId);
      assert.strictEqual(share.status, "unbound", "Share must remain unbound in production fallback mode");
    } finally {
