...
| `src/writer/`    | 22,624 | Song/lyrics generation. `v3/` is the live story engine; `songwriter.js` is the live lyrics layer; **`v2/` is DEAD (zero JS files)**. Root 7 has broken the songwriter↔v3 contract-validation cycle through `song-contract.js`, moved songwriter prompt budgeting to `songwriter/prompt-budget.js`, moved poem readiness to `v3/quality/poem-readiness.js`, moved question targeting to `v3/quality/question-targeting.js`, moved Labov gap analysis to `v3/quality/labov-gap-analysis.js`, moved semantic story package repair to `v3/semantic-story-package.js`, moved runtime question helpers to `v3/runtime-questions.js`, moved ready-confirmation text to `v3/ready-confirmation.js`, moved turn-decision result assembly and `resolveTurnDecision` orchestration to `v3/turn-decision.js`, and centralized V3 sentence splitting in `v3/utils.js`. Legacy reasoner path remains as fallback; `v3/index.js`, `songwriter.js`, and remaining `quality.js` legacy slot-analysis logic are still large. | 🟡 improving; god-files remain |
...
`buildServer()` (3,092 lines), `runner.js` `startJobRunner` (1,709-line closure inside a 3,934-line file), `admin-service.js` (2,539 lines / 94 detected method-like entries), `writer/v3/index.js` (2,829), `songwriter.js` (3,569), `writer/v3/quality.js` (1,926). The former `routes/admin.js` god controller is now a 605-line registrar shell over 28 admin route modules, and Root 7 has moved multiple writer helper clusters into leaf modules, so remaining D2 risk has shifted from route-handler concentration to service, workflow, and unresolved writer orchestration god files.
