...
- **The funnel is a page of porizo.co, not a standalone app (per Ambrose, 2026-07-15).** `/create` renders under the site's own chrome: reuse the `.nav--static` header from `public/styles/main.css` verbatim (logo · How it works · Pricing · Blog) with ONE funnel modification — the nav CTA is a quiet **"Sign in"** (magic-link web login), never "Get the app" mid-purchase. Landing/SEO entry points (hero CTA, "Start a song for…" occasion pills) flow into `/create` with `?occasion=` prefill — same tab, same site, no context break. Site footer renders on the `/create` entry state only; it drops once the first answer commits (focus mode). Dim pages (S6 player, `/play`) reduce chrome to the brand mark — the theater dimming justifies it.
- **Centering is a hard contract:** the `--col: 480px` column sits horizontally centered in the viewport at EVERY width (`.shell { width:100%; margin-inline:auto; }` — guard against ancestor flex/width leaks; the desktop ambient radial fills the surrounding room). A left-hugging column is a review-failing bug. Verified reference: `flow-stacked.html` at 1440×900.
- Browser back = collapse-reopen of the previous step (history entries per step).
