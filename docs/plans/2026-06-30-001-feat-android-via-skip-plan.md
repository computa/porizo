...
1. **The backend is already the authority, not a no-op dependency.** ~23k LOC across 18 route files (`src/routes/`) — auth, tracks, story/create, rendering jobs, gifts, sharing, billing, enrollment — remains server-authoritative. Android reuses the same contracts, but the plan must add server work for Android App Links, provider-aware push send (FCM routing — the `devices.platform` column already exists), passwordless email, Play consumables, and Android claim/device-trust attestation.
...
- KTD3. **Reuse the backend as the authority; do not call it verbatim.** No second backend. The actual backend work is a set of production migrations around otherwise-shared contracts:
  - **Passwordless email is NOT yet supported server-side.** `src/routes/auth.js` has phone-OTP, password login, and social (`/auth/social`) — but no email magic-link/OTP path (verified: only SMS/phone OTP at `/auth/phone/send-code`). Email magic-link is chosen as the Android recipient primary login over the existing phone-OTP because a gift recipient typically arrives from an emailed/messaged link without a verified phone on file, and SMS adds cost, country-coverage gaps, and friction at the exact moment we need the claim to complete. This is a deliberate product trade, accepting that it is **new, security-sensitive server work**, not shared-client work. It is specified as its own unit (U8a) following the existing phone-registration-token pattern (hash-at-rest, single-use, short TTL, enumeration-neutral, rate-limited), delivered over a **verified App Link** (never the `porizo://` custom scheme).
  - **Google Play validators partly exist.** `POST /billing/receipt/google` + `src/services/google-receipt-validator.js` already validate Google subscriptions, but the billing route is **subscription-only** today (it calls `handleGoogleSubscriptionValidation`, not the one-time path). The validator **already exposes `verifyPurchase` for one-time products** (`google-receipt-validator.js:279`, via the `purchases/products/` API) — so the consumable work is _wiring that existing helper into a new route plus ledger flow_, not writing validation from scratch: product catalog mapping, replay/cross-account/concurrency protection, transaction identity, wallet credit in one transaction, and acknowledgement/consume semantics.
  - **Push/device registration is new send-routing, not a schema migration.** The `devices` table **already has a `platform` column** (`device-repository.js:21,35`), so Android FCM needs **no schema change to store the token**. The genuinely-new work is provider-aware _send_ logic: APNs-vs-FCM routing, longer FCM token validation, ownership transfer, stale-token cleanup, and APNs+FCM dual-send. OneSignal is the current delivery layer and must be reconciled (external-ID/tag sync vs. product push transport).
...
- KTD4. **Push is dual-stack first; iOS APNs migration is not on the recipient critical path.** Android gets FCM support without forcing existing iOS users off APNs. The `devices` table already carries a `platform` column, so the first backend step is provider-aware _send_ (`apns`/`fcm` routing, longer FCM token validation, last seen, stale cleanup) rather than a token-schema migration. Existing iOS APNs continues until a separate migration unit proves dual-send metrics, rollback, token re-registration, and OneSignal external-ID/tag reconciliation. This keeps the Android recipient fix from being blocked by a live-user iOS notification migration.
...
- Approach:
  ...
  - **The DB binding mechanism is already platform-neutral** — `share_tokens.bound_device_id` is compared against a **client-asserted** `device_id` (`sharing.js:1128`, from body or `x-device-id`), so an Android client supplying a UUID already binds. The net-new work is **attestation**, not plumbing: today there is zero server-side proof of that identity. Add Play Integrity + App Set ID token validation (nonce/freshness/replay checks) so the bound `device_id` is server-trusted, not just client-claimed.
...
- Approach:
  ...
  - The `devices` table already has a `platform` column, so add only the missing provider/environment fields (not a token-schema rebuild), plus longer FCM token validation, ownership transfer, stale cleanup, and token refresh handling.
...
In scope: native Android app via Skip Fuse **only if Gate A/B pass**; Android Recipient MVP before creator-side parity; backend kept as the shared authority with explicit server work for provider-aware FCM push send (no token-schema migration — `devices.platform` exists), Android App Links, passwordless email auth, Android device-trust attestation, and Google consumable receipts; iOS app purification/modularization as a necessary shared-code step. iOS-side StoreKit stays untouched. Existing iOS APNs product push stays active until a separate, measured migration proves FCM is better.
...
- Backend/security: FCM needs new provider-aware send (the `devices.platform` column already exists, so this is send-routing, not a token-schema migration), dual APNs/FCM send, Android App Links, Play Integrity/App Set ID server attestation over the already-platform-neutral device binding, and wiring the existing `verifyPurchase` one-time validator into a new consumable route with replay/consume semantics.
...
- Backend work: provider-aware FCM/APNs send (using the existing `devices.platform` column), `assetlinks.json`, passwordless email auth, Android device-trust attestation, Google consumable route/catalog mapping. Existing `/billing/receipt/google` (subscriptions + `verifyPurchase` one-time helper) and `/auth/social` are reused where valid.
