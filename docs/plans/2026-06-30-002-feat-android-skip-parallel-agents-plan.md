...
- [ ] **U1** (Gate A) — Skip Fuse spike in `spikes/skip-fuse-spike/`, fill thresholds on hardware.
- [ ] **U2** (Gate A) — Platform research; each question gets link/conclusion/owner/blocks-flag.
- [ ] **Gate A verdict** — owner writes "Skip" / "Compose fallback" into `android-skip-gate-a-findings.md`. **Blocks all of S1/S2/S4.**
