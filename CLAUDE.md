...
- This repo's code (registered as `gstack-code-porizo` source).
- `~/.gstack/` curated memory (registered as `gstack-brain-ao` source via
...
- "What did we decide last time?" / past plans, retros, learnings:
  `gbrain search "<terms>" --source gstack-brain-ao`
