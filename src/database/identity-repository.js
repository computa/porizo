...
function createIdentityRepository(db) {
  async function transaction(callback) {
    if (typeof db.transaction !== "function") {
      throw new Error(
        "Identity repository mutations require database transaction support",
      );
    }
  ...
  async function findActiveUserByProvider(
    provider,
    providerUserId,
    options = {},
  ) {
  ...
  async function listActiveAuthProvidersForUser(userId) {
    return db
      .prepare(
        "SELECT provider, provider_user_id FROM user_auth_providers WHERE user_id = ? AND status = 'active'",
      )
      .all(userId);
  }
  ...
  async function getContactTypeForUser(contactId, userId) {
    return db
      .prepare("SELECT type FROM user_contacts WHERE id = ? AND user_id = ?")
  ...
  async function setPrimaryContact(contactId) {
    return db
      .prepare("UPDATE user_contacts SET is_primary = true WHERE id = ?")
      .run(contactId);
  }
  ...
  async function updateUserContactMirrors({
    userId,
    email,
    emailVerified,
    phoneNumber,
  ...
  async function updateIdentityLastUsed(identityId, lastUsedAt) {
    return db
      .prepare("UPDATE user_auth_providers SET last_used_at = ? WHERE id = ?")
  ...
  async function findVerifiedContactConflict(
    type,
    valueNormalized,
    excludeUserId = null,
  ) {
