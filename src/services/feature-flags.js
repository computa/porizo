...
const DEFAULTS = {
  ...
  // One authority for the Etsy wedge and future provider automation.
  etsy_fulfilment_mode: "off",
  ...
  // Suno voice persona is GA on `version3`. Default ON intentionally — this is
  // an opt-OUT kill switch, not a staged-rollout opt-in gate. Set this flag to
  // `false` in feature_flags table to disable persona enqueueing during a
  // Suno-side outage. The route-level read is in src/routes/enrollment.js.
  // (Tracked under H11 in tasks/codex-review-72h.md — staged opt-in not
  // adopted because the feature is already shipped to TestFlight build 110.)
...
const FLAG_METADATA = {
  seedvc_cfg_rate: {
    ...
    label: "CFG Rate",
    description:
      "Voice fidelity vs natural singing balance. Lower values produce more natural singing, higher values increase voice similarity.",
    type: "number",
    min: 0.1,
    max: 1.0,
    step: 0.05,
  ...
  seedvc_diffusion_steps_preview: {
    ...
    description:
      "Number of diffusion steps for preview renders. Higher values produce better quality but take longer.",
    type: "number",
    min: 10,
    max: 200,
    step: 10,
  ...
  seedvc_diffusion_steps_full: {
    ...
    description:
      "Number of diffusion steps for full renders. Higher values produce better quality but take longer.",
    type: "number",
    min: 25,
    max: 300,
    step: 25,
  ...
  timbre_blend_ratio: {
    ...
    description:
      "Blend between AI vocals and converted vocals. 0.0=pure AI, 1.0=full conversion (legacy). Sweet spot: 0.15-0.4.",
    type: "number",
    min: 0.0,
    max: 1.0,
    step: 0.05,
  ...
  timbre_cfg_rate: {
    ...
    description:
      "CFG rate for voice conversion when timbre blending is active. Lower = more natural singing quality preserved.",
    type: "number",
    min: 0.1,
    max: 0.8,
    step: 0.05,
  ...
  timbre_blend_strategy: {
    ...
    description:
      "Algorithm for blending AI and converted vocals. Amplitude=volume mix, Spectral=frequency-band split, Doubling=compressed subliminal double, Formant=EQ transfer.",
    type: "string",
    options: [
      ...
      {
        value: "perceptual_primary",
        label: "Perceptual Primary (User Dominant)",
      },
    ],
  },
  spectral_crossover_low_hz: {
    ...
    description:
      "Low frequency boundary for spectral crossover. Below this, AI vocal is used exclusively.",
    type: "number",
    min: 100,
    max: 500,
    step: 50,
  ...
  spectral_crossover_high_hz: {
    ...
    description:
      "High frequency boundary for spectral crossover. Above this, AI vocal is used exclusively.",
    type: "number",
    min: 2000,
    max: 5000,
    step: 250,
  ...
  spectral_mid_blend_ratio: {
    ...
    description:
      "How much converted vocal to blend in the formant band (300-3kHz). 0=pure AI, 1=pure converted.",
    type: "number",
    min: 0.0,
    max: 1.0,
    step: 0.05,
  ...
  doubling_level: {
    ...
    description:
      "Volume level of the compressed vocal double. 0.05=barely audible, 0.20=noticeable.",
    type: "number",
    min: 0.02,
    max: 0.3,
    step: 0.02,
  ...
  doubling_presence_cut_freq: {
    ...
    description:
      'Center frequency for presence EQ cut on the double. Removes "second voice" articulation.',
    type: "number",
    min: 2000,
    max: 6000,
    step: 500,
  ...
  doubling_presence_cut_gain: {
    ...
    description:
      "Gain reduction at presence frequency. More negative = more cut.",
    type: "number",
    min: -15,
    max: 0,
    step: 1,
  ...
  formant_transfer_strength: {
    ...
    description:
      "How strongly to apply the formant EQ correction. 0=no effect, 1=full transfer.",
    type: "number",
    min: 0.0,
    max: 1.0,
    step: 0.1,
  ...
  perceptual_ai_influence: {
    ...
    description:
      "How much AI vocal bleeds through when user voice is silent. 0=none, 0.5=max allowed.",
    type: "number",
    min: 0.0,
    max: 0.5,
    step: 0.05,
  ...
  perceptual_ducking_strength: {
    ...
    description:
      "How aggressively AI ducks when user sings. 0=no ducking, 1=full ducking.",
    type: "number",
    min: 0.0,
    max: 1.0,
    step: 0.05,
  ...
  vocal_polish_enabled: {
    ...
    description:
      "Apply post-processing to Seed-VC output to reduce harshness and add warmth.",
    type: "boolean",
  },
  ...
  vocal_polish_de_harsh_gain: {
    ...
    description:
      "How much to cut at the harsh frequency. More negative = more cut.",
    type: "number",
    min: -8,
    max: 0,
    step: 1,
  ...
  vocal_polish_compression_threshold: {
    ...
    description:
      "Compression threshold (0-1). Lower = more compression applied.",
    type: "number",
    min: 0.05,
    max: 0.3,
    step: 0.05,
  ...
  vocal_polish_de_ess_freq: {
    ...
    description:
      "Center frequency for sibilance reduction. Sibilance typically lives at 5-8kHz.",
    type: "number",
    min: 4000,
    max: 9000,
    step: 500,
  ...
  vocal_polish_de_ess_gain: {
    ...
    description:
      "How much to cut sibilance. More negative = more reduction. 0 = disabled.",
    type: "number",
    min: -12,
    max: 0,
    step: 1,
  ...
  vocal_polish_de_ess_width: {
    ...
    description:
      "Bandwidth of the de-essing EQ. Wider = affects more frequencies around center.",
    type: "number",
    min: 0.5,
    max: 4.0,
    step: 0.5,
  ...
  seedvc_auto_f0_adjust: {
    ...
    description:
      "Automatically adjust fundamental frequency to match target voice pitch range.",
    type: "boolean",
  },
  seedvc_f0_condition: {
    category: "voice_conversion",
    ...
    description:
      "Condition on source F0 contour during conversion. Preserves original pitch dynamics.",
    type: "boolean",
  },
  seedvc_pitch_shift: {
    category: "voice_conversion",
  ...
  voice_conversion_provider: {
    ...
    description:
      "Which provider to use for voice conversion. seedvc=free Seed-VC, elevenlabs=ElevenLabs Voice Changer (~$0.10/preview).",
    type: "select",
  ...
  suno_voice_persona_model: {
    ...
    description:
      "Suno model used for user-voice persona song generation after the provider persona exists.",
    type: "select",
  ...
  suno_voice_persona_persona_model: {
    ...
    description:
      "Persona mode sent with personaId for final Suno generation. voice_persona is voice-focused.",
    type: "string",
  ...
  suno_voice_persona_audio_weight: {
    ...
    description:
      "Reference-audio weight for the persona preparation cover/upload path.",
    type: "number",
    min: 0.0,
    max: 1.0,
    step: 0.05,
  ...
  suno_voice_persona_enabled: {
    ...
    description:
      "Backend kill switch for queueing Suno voice-persona profile creation.",
    type: "boolean",
  },
  my_voice_enabled: {
    category: "voice_conversion",
    ...
    description:
      'Global toggle for personalized voice. OFF hides "My Voice" in clients and routes user_voice requests to AI voice.',
    type: "boolean",
  },
  ...
  gift_prepay_enforced: {
    ...
    description:
      "Requires reservation-first flow before gift content creation/finalization.",
    type: "boolean",
  },
  gift_sms_enabled: {
    category: "developer",
  ...
  gift_require_app_claim: {
    ...
    description:
      "For gifted shares, disables web playback and requires app claim before access.",
    type: "boolean",
  },
  elevenlabs_stability: {
    ...
    description:
      "Voice consistency. LOW (0.3-0.5) preserves melodic contour for singing. HIGH (0.8-1.0) sounds flat/robotic.",
    type: "number",
    min: 0.0,
    max: 1.0,
    step: 0.05,
  ...
  elevenlabs_similarity_boost: {
    ...
    description:
      "How closely to match the cloned voice. Higher = stronger voice match but may amplify artifacts.",
    type: "number",
    min: 0.0,
    max: 1.0,
    step: 0.05,
  ...
  voice_enrollment_sung_threshold_relaxation: {
    ...
    description:
      "Whether to use relaxed thresholds for sung audio quality checks.",
    type: "boolean",
  },
  ...
  voice_enrollment_ios_realtime_feedback: {
    ...
    description:
      "Enable realtime audio level feedback during enrollment on iOS.",
    type: "boolean",
  },
  show_design_screens: {
    category: "developer",
    ...
    description:
      "Make design preview screens accessible in TestFlight and debug builds. Never visible on App Store.",
    type: "boolean",
  },
  ...
  free_tier_poems_grant: {
    ...
    description:
      "Number of poems granted to new free accounts on creation. Set to 0 to disable.",
    type: "number",
    min: 0,
    max: 10,
    step: 1,
...
async function getFeatureFlag(
  ...
  try {
    ...
    console.error(
      `[FeatureFlags] FF001_DB_READ_ERROR: Failed to get flag ${flagId}:`,
      e.message,
    );
    if (throwOnError) {
      throw new Error(
        `FF001_DB_READ_ERROR: Database error reading flag ${flagId}: ${e.message}`,
      );
    }
  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        console.error(
          `[FeatureFlags] FF002_PARSE_ERROR: Failed to parse value for flag ${row.id}:`,
          e.message,
        );
    ...
    console.error(
      "[FeatureFlags] FF003_BATCH_READ_ERROR: Failed to batch get flags:",
      e.message,
    );
    if (throwOnError) {
      throw new Error(
        `FF003_BATCH_READ_ERROR: Database error reading flags: ${e.message}`,
      );
    }
...
async function setFeatureFlag(
  ...
  try {
    ...
    console.error(
      `[FeatureFlags] FF004_WRITE_ERROR: Failed to set flag ${flagId}:`,
      e.message,
    );
    throw new Error(
      `FF004_WRITE_ERROR: Failed to save flag ${flagId}: ${e.message}`,
    );
  }
}
...
async function getVoiceEnrollmentConfig(db) {
  const flagIds = Object.keys(DEFAULTS).filter((k) =>
    k.startsWith("voice_enrollment_"),
  );
