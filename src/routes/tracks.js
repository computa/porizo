...
function registerTrackRoutes(
  ...
  async function consumeSongEntitlementInTransaction(
    query,
    { userId, trackId, trackVersionId, consumedAt, fundingSource = "standard" },
  ) {
    if (consumedAt) {
    ...
    const spendInTransaction =
      typeof subscriptionManager.spendSongInTransaction === "function"
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (body.relationship_type)
        storyContext.relationship_type = body.relationship_type;
      if (body.years_known) storyContext.years_known = body.years_known;
      if (body.specific_memory)
        storyContext.specific_memory = body.specific_memory;
      if (body.special_phrases)
        storyContext.special_phrases = body.special_phrases;
      if (body.what_makes_them_special)
        storyContext.what_makes_them_special = body.what_makes_them_special;
      // AI-generated follow-up question answers from wizard
      if (
        Array.isArray(body.memory_answers) &&
        body.memory_answers.length > 0
      ) {
  ...
  app.patch("/tracks/:id/voice_mode", async (request, reply) => {
    ...
    if (!["user_voice", "ai_voice"].includes(voice_mode)) {
      sendError(
        ...
        400,
        "INVALID_VOICE_MODE",
        "voice_mode must be 'user_voice' or 'ai_voice'",
      );
      return;
    }
    ...
    async (request, reply) => {
      ...
      if (existing) {
        sendError(
          ...
          409,
          "DUPLICATE_PARAMS",
          "Version with identical params already exists.",
          {
    ...
    async (request, reply) => {
      ...
      if (trackVersion.moderation_status === "blocked") {
        sendError(
          ...
          403,
          "MODERATION_BLOCKED",
          "Track version blocked by moderation.",
          {
      ...
      if (trackVersion.lyrics_status !== "approved") {
        sendError(
          ...
          409,
          "LYRICS_NOT_APPROVED",
          "Lyrics must be approved before rendering.",
        );
        return;
      }
      if (trackVersion.status === "preview_ready" && trackVersion.preview_url) {
        reply.code(200).send({
          ...
          poll_url: trackVersion.preview_job_id
            ? `/jobs/${trackVersion.preview_job_id}`
            : null,
        });
        return;
      }
      ...
      if (!existingJob) {
        const latestFailedPreviewJob = await findLatestFailedJobForVersion(
          trackVersion.id,
          "preview_render",
        );
        if (
          isTerminalTrackFailureStatus(trackVersion.status) &&
          latestFailedPreviewJob
        ) {
      ...
      if (!existingJob) {
        existingJob = await findActiveJobForVersion(
          trackVersion.id,
          "preview_render",
        );
        if (existingJob) {
      ...
      if (existingJob && isTerminalFailedJobStatus(existingJob.status)) {
        const lyricsUpdatedAt = toTimestamp(trackVersion.lyrics_updated_at);
        const failureAt =
          toTimestamp(existingJob.completed_at) ||
          toTimestamp(existingJob.updated_at);
        const lyricsChangedSinceFailure =
          Number.isFinite(lyricsUpdatedAt) &&
          Number.isFinite(failureAt) &&
          lyricsUpdatedAt > failureAt;
      ...
      try {
        ...
        if (
          ...
          sendError(
            ...
            "Insufficient songs remaining to start this song.",
          );
          return;
        }
        if (txError.message === "ALREADY_RENDERING") {
          const fallbackJob = await findActiveJobForVersion(
            trackVersion.id,
            "preview_render",
          );
          if (fallbackJob) {
          ...
          sendError(
            ...
            409,
            "ALREADY_RENDERING",
            "Preview render already in progress.",
          );
          return;
        }
        if (txError.message === "SPEND_IN_TX_UNAVAILABLE") {
          console.error(
            "[Billing] subscriptionManager.spendSongInTransaction is unavailable.",
          );
        }
        console.error(
          "[Billing] Preview render entitlement transaction failed:",
          txError.message,
        );
        sendError(
          reply,
          500,
          "BILLING_ERROR",
          "Failed to process song entitlement. Please try again.",
        );
        return;
      }
  ...
  app.post(
    "/tracks/:id/versions/:version/render_full",
    async (request, reply) => {
      const userId = await requireUserId(request, reply);
      if (!userId) {
        return;
      ...
      try {
        ...
        if (
          ...
          sendError(
            ...
            402,
            "INSUFFICIENT_CREDITS",
            "Insufficient songs remaining to continue this song.",
          );
          return;
        }
        if (txError.message === "ALREADY_RENDERING") {
          ...
          sendError(
            ...
            409,
            "ALREADY_RENDERING",
            "Track is already being rendered.",
          );
          return;
        }
        if (txError.message === "SPEND_IN_TX_UNAVAILABLE") {
        ...
        console.error(
          `[Billing] Transaction failed for user ${userId}:`,
          txError.message,
        );
        sendError(
          reply,
          500,
          "BILLING_ERROR",
          "Failed to process billing. Please try again.",
        );
        return;
      }
  ...
  app.post("/tracks/:id/versions/:version/retry", async (request, reply) => {
    ...
    if (result.conflict) {
      sendError(
        ...
        409,
        "JOB_STATUS_CHANGED",
        "Job status changed before retry could be applied. Please try again.",
      );
      return;
    }
    console.log(
      `[retry] Job re-queued: jobId=${result.job.id}, trackVersionId=${trackVersion.id}, workflow=${workflowType}`,
    );
    reply.code(202).send({
  ...
  app.post("/tracks/:id/versions/:version/cancel", async (request, reply) => {
    ...
    const activePreview = await findActiveJobForVersion(
      trackVersion.id,
      "preview_render",
    );
    const activeFull = await findActiveJobForVersion(
      trackVersion.id,
      "full_render",
    );
    ...
    try {
      ...
      console.log(
        `[cancel] Render cancelled: jobId=${activeJob.id}, trackVersionId=${trackVersion.id}`,
      );
      reply.send({
        cancelled: true,
        job_id: activeJob.id,
      ...
      if (err.code === "NO_ACTIVE_RENDER") {
        sendError(
          ...
          409,
          "NO_ACTIVE_RENDER",
          "Render already completed or cancelled.",
        );
        return;
      }
      console.error("[cancel] Cancel render failed:", {
        trackVersionId: trackVersion.id,
        error: err.message,
      });
  ...
  app.post("/tracks/:id/versions/:version/reroll", async (request, reply) => {
    ...
    sendError(
      ...
      410,
      "FEATURE_RETIRED",
      "Reroll has been retired. Edit your story and create a new song instead.",
    );
  });
  ...
  app.post(
    "/tracks/:id/versions/:version/lyrics/generate",
    { schema: schemas.generateLyrics },
    async (request, reply) => {
      const userId = await requireUserId(request, reply);
      if (!userId) {
        return;
      ...
      try {
        ...
        if (err && err.code === "LYRICS_QUALITY_LOW") {
          sendError(
            ...
            422,
            "LYRICS_QUALITY_LOW",
            "Generated lyrics were not strong enough to use yet.",
            {
              quality_score: Number.isFinite(err.quality_score)
                ? err.quality_score
                : null,
            },
          );
        ...
        if (err && err.code === "LYRICS_FIDELITY_LOW") {
          sendError(
            ...
            422,
            "LYRICS_FIDELITY_LOW",
            "Generated lyrics did not stay faithful enough to the story.",
            {
      ...
      const lyricsStatus = validation.hasAnchor
        ? result.lyrics_status
        : "needs_anchor";
  ...
  app.post(
    "/tracks/:id/versions/:version/lyrics/approve",
    async (request, reply) => {
      const userId = await requireUserId(request, reply);
      if (!userId) {
        return;
      ...
      if (!limit.allowed) {
        sendError(
          ...
          429,
          "RATE_LIMITED",
          "Lyrics approval rate limit reached.",
          {
            retry_after: limit.reset_at,
          },
        );
  ...
  app.post("/tracks/:id/share", async (request, reply) => {
    ...
    if (body.og_variant !== undefined) {
      ...
      const providedVariant =
        rawVariant === null ? "" : String(rawVariant).trim();
    ...
    if (!trackVersion.preview_url && !trackVersion.full_url) {
      sendError(
        ...
        409,
        "TRACK_NOT_READY",
        "Track version is not ready to share.",
      );
      return;
    }
