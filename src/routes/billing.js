...
function registerBillingRoutes(
  ...
  async function handleGoogleSubscriptionValidation({
    ...
    const resolvedSubscriptionId =
      subscriptionId || validation.raw?.lineItems?.[0]?.productId || null;

    if (!resolvedSubscriptionId) {
      const err = new Error(
        "Could not resolve Google subscription product ID.",
      );
      err.code = "INVALID_RECEIPT";
      throw err;
    }
  ...
  app.get("/billing/gift-wallet", async (request, reply) => {
    ...
    try {
      const summary = await getGiftWalletSummary(
        userId,
        Math.max(1, Math.min(100, parseInt(request.query?.limit) || 20)),
      );
      reply.send({
        ...
        .transaction(async (query) => {
          try {
            ...
            if (
              ...
              dedupReceipt.product_id !== validation.productId
            ) {
  ...
  app.post("/billing/receipt/google", async (request, reply) => {
    ...
    try {
      ...
      const { purchase_token, purchaseToken, subscription_id, subscriptionId } =
        request.body || {};
      ...
      if (!resolvedPurchaseToken || !resolvedSubscriptionId) {
        sendError(
          ...
          400,
          "MISSING_PARAMS",
          "purchase_token and subscription_id are required.",
        );
        return;
      }
    ...
    try {
      ...
      for (const mapping of productMappings.values()) {
        ...
        if (mapping.platform === "apple" || mapping.platform === "google") {
          if (
            mapping.billing_period === "monthly" ||
            mapping.billing_period === "annual"
          ) {
            planEntry[mapping.platform][mapping.billing_period] =
              mapping.product_id;
          }
        }
  ...
  app.post("/billing/restore", async (request, reply) => {
    ...
    const effectiveTransactionId = transactionId || legacyTransactionId;
    const effectivePurchaseToken =
      purchaseToken || legacyPurchaseToken || effectiveTransactionId;
    const effectiveSubscriptionId =
      subscriptionId || legacySubscriptionId || null;

    if (
      !platform ||
      (platform === "apple" && !effectiveTransactionId) ||
      (platform === "google" && !effectivePurchaseToken)
    ) {
      sendError(
        reply,
        400,
    ...
    try {
      ...
      if (platform === "apple") {
        ...
        validation = await appleValidator.verifyTransaction(
          effectiveTransactionId,
        );
      } else {
        if (!googleValidator.isConfigured()) {
  ...
  app.post("/billing/trial/activate", async (request, reply) => {
    ...
    if (!limit.allowed) {
      sendError(
        ...
        429,
        "RATE_LIMITED",
        "Trial activation rate limit reached.",
        {
  ...
  app.post("/billing/webhooks/apple", async (request, reply) => {
    ...
    try {
      const result =
        await appleWebhookHandler.processNotification(signedPayload);
      ...
      if (!result.success) {
        ...
        if (!isInvalidPayload) {
          ...
          console.warn(
            "[Apple Webhook] Invalid payload received; acknowledging to prevent retry storms",
          );
        }
  ...
  app.post("/billing/webhooks/google", async (request, reply) => {
    ...
    if (!webhookSecret) {
      console.warn(
        "[SecurityGuard:WebhookAuth] GOOGLE_WEBHOOK_SECRET not configured — rejecting webhook",
      );
    ...
    if (!token || token !== webhookSecret) {
      console.warn(
        "[SecurityGuard:WebhookAuth] Google webhook rejected — invalid token",
      );
    ...
    try {
      ...
      if (!subNotification?.purchaseToken) {
        reply.send({
          received: true,
          processed: false,
          message: "Ignored non-subscription Google webhook",
        });
        return;
      }
    ...
    try {
      ...
      if (syncAllSubscriptions) {
        ...
        for (const validation of subscriptions) {
          if (
            !validation?.valid ||
            (validation.type && validation.type !== "subscription")
          ) {
            continue;
          }
          try {
      ...
      if (syncResults.length === 0) {
        const firstError =
          syncErrors[0]?.error || "No subscriptions were synced.";
      ...
      await addAuditEntry({
        ...
        resourceType: "subscription",
        resourceId:
          activeSubscription?.id || syncResults[0].subscriptionId || null,
        metadata: {
  ...
  app.post("/admin/billing/grant-songs", async (request, reply) => {
    ...
    if (!targetUserId || !amount || amount <= 0) {
      sendError(
        ...
        400,
        "INVALID_PARAMS",
        "targetUserId and amount (positive) are required.",
      );
      return;
    }
    ...
    try {
      ...
      if (result.changes === 0) {
        sendError(
          ...
          404,
          "USER_NOT_FOUND",
          "No entitlements found for user.",
        );
        return;
      }
      ...
      console.log(
        `[Admin] Reset preview count for user ${targetUserId} by ${admin.email}`,
      );
      reply.send({
        success: true,
        userId: targetUserId,
  ...
  app.put("/admin/plans/:planId", async (request, reply) => {
    ...
    try {
      const result = await planConfigService.updatePlan(
        planId,
        filteredUpdates,
      );
  ...
  app.post("/admin/plans", async (request, reply) => {
    ...
    if (!name || !tier || songs_per_month === undefined) {
      sendError(
        ...
        400,
        "MISSING_FIELDS",
        "name, tier, and songs_per_month are required.",
      );
      return;
    }
    ...
    if (typeof features_json === "string") {
      try {
        featuresArray = JSON.parse(features_json);
      } catch {
        featuresArray = null;
      }
    }
  ...
  app.post("/admin/plans/:planId/products", async (request, reply) => {
    ...
    if (!platform || !product_id || !billing_period) {
      sendError(
        ...
        400,
        "MISSING_FIELDS",
        "platform, product_id, and billing_period are required.",
      );
      return;
    }
  ...
  app.get("/admin/billing/preflight", async (request, reply) => {
    ...
    try {
      ...
      const activePaidPlans = plans.filter(
        (plan) => plan.is_active && plan.tier !== "free",
      );
      for (const plan of activePaidPlans) {
        const planMappings = appleMappings.filter(
          (mapping) => mapping.plan_id === plan.id,
        );
        const monthly = planMappings.find(
          (mapping) => mapping.billing_period === "monthly",
        );
        const annual = planMappings.find(
          (mapping) => mapping.billing_period === "annual",
        );
      ...
      if (!appleValidatorConfigured) {
        issues.push({
          ...
          message:
            "Apple receipt validator is not fully configured (missing key/issuer/private-key/bundle-id).",
          details: null,
        });
      }
      ...
      if (!expectedBundleId) {
        warnings.push({
          ...
          message:
            "No expected_bundle_id query parameter supplied; bundle match check was skipped.",
        });
      }
...
module.exports = { registerBillingRoutes, buildEntitlementsPayload };
